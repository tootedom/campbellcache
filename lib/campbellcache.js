    ...
    this.scheduleValueComputationForDisabledCache = function(key,supplier,observableDict,metricsReporter) {
        var obsy = new Rx.Observable.create(function(observer) {
            ...
            try {
                ...
                if ( observableSupplier == null || !(observableSupplier instanceof Rx.Observable) ) {
                    _completeWithError(observer,key,new Error(Constants.INVALID_SUPPLIER_FUNCTION_ERROR_MSG),
                                    observableDict,metricsReporter,timer);
                    return null;
                }
            } catch(err) {
                _completeWithError(observer,key,err,observableDict,metricsReporter,timer);
                return null;
            }
 ...
 * @param {Observable function()} supplier : A function that is executed that returns an Observable 
