...
function _completeFromSupplier(metricsReporter,memcachedClient,
    ...
    const sub = observableSupplier.subscribe(
        (value) => {
            ...
            var isCacheableValue = options.isSupplierValueCachablePredicate(value);
            if(isNotNullResults && isCacheableValue) {
                ...
                if(options.waitForMemcachedSet) {
                    memcachedClient.set(key,value,options,observer);
                } else {
                    memcachedClient.set(key,value,options);
                    observer.next(new CacheItem(key,value,false));
                    observer.complete();
                }
                delete observableCache[key];
                ...
                logger.debug("Cache Value cannot be cached.  It has to be either not null:({0}), or cachable as determine by predicate:({0}). " +
                        "Therefore, not storing in memcached", isNotNullResults, isCacheableValue);
                observer.next(new CacheItem(key,value,false));
                observer.complete();
                delete observableCache[key];
            }
...
CampbellCache.prototype.set = function(key,supplier,options) {
    var defaultOptions = {
        ttl : 0,
        isSupplierValueCachablePredicate : OK_PREDICATE,
        waitForMemcachedSet : false,
        compress : false
    };

    const setOptions = Object.assign({},defaultOptions,options);
...
CampbellCache.prototype.apply = function(key, supplier, options) {
    var defaultOptions = {
        ttl : 0,
        isSupplierValueCachablePredicate : OK_PREDICATE,
        isCachedValueValidPredicate : OK_PREDICATE,
        waitForMemcachedSet : false,
        compress : false
    };
            ...
            var obsy = new Rx.Observable.create(function(observer) {
                ...
                const sub = distributedCacheObs.subscribe((cachedItem) => {
                    if(cachedItem.isNotFromCache()) {
                        ...
                        if(setOptions.isCachedValueValidPredicate(cachedItem.value())) {
