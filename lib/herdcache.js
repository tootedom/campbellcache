    ...
    this.scheduleValueComputationForDisabledCache = function(key,supplier,observableDict) {
        var obsy = new Rx.Observable.create(function(observer) {
            ...
            try {
                ...
                console.log("What is it: " + observableSupplier);
                if ( observableSupplier == null ) {
                    _completeWithError(observer,key,new Error("Supplier function did not return an Observable"),
...
function _completeWithError(observer, key, err, observableCache,metricsRecorder, timer) {
    ...
    metricsReporter.setDuration(CacheMetricStrings.CACHE_TYPE_SUPPLIER_FUNCTION_FAILURE_TIMER,timer.inspect());
    metricsReporter.incrementCounter(CacheMetricStrings.CACHE_TYPE_SUPPLIER_FUNCTION_FAILURE_COUNTER);
}
