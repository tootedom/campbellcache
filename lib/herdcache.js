...
function HerdCache(options) {
    options.autodiscovery = options.autodiscovery || true;
    options.autodiscovery_timeoutInMs = options.autodiscovery_timeoutInMs;
    options.autodiscovery_intervalInMs = options.autodiscovery_intervalInMs;
    options.autodiscovery_oldClientTTL = options.autodiscovery_oldClientTTL || 120000;
    ...
    this.metrics = new MetricRecorder({
        ...
        registries: options.metrics_registries || null
    });

    options.memcached_opts['metricsrecorder'] = this.metrics;

    options.memcached_opts.memcached = options.memcached_opts.memcached || {}

    this.client = this._observableMemcacheFactory(options.hosts,options.memcached_opts);
    this.herdDict = {};
    ...
    if(options.autodiscovery) {
        ...
        this.autodiscovery.register(_updateClient(this,options.memcached_opts,
                                                  options.autodiscovery_time_in_ms_before_closing_old_client));
    } else {
